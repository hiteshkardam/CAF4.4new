...
static void mdss_dsi_vddio_switch(struct mdss_dsi_ctrl_pdata *ctrl_pdata, int enable)
{
	...
	if (enable) {
		...
		regulator_set_load(in_vreg[vddio].vreg,
		...
		ret = regulator_set_load(in_vreg[vddio].vreg, in_vreg[vddio].enable_load);
		if (ret == 0)
...
static int mdss_dsi_panel_power_off_hx8396c2(struct mdss_panel_data *pdata)
{
	...
	regulator_set_load(in_vreg[ibb].vreg, in_vreg[ibb].disable_load);
	regulator_disable(in_vreg[ibb].vreg);
	regulator_set_load(in_vreg[lab].vreg, in_vreg[lab].disable_load);
