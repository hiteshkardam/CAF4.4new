...
static int qpnp_wled_set_level(struct qpnp_wled *wled, int level)
{
	...
	if (wled->hyb_thres_low_wm) {
		/* PMIC Noise workaround */
		rc = qpnp_wled_read_reg(wled, QPNP_WLED_HYB_THRES_REG(wled->sink_base), &reg0);
		if (rc < 0) {
			...
			rc = qpnp_wled_write_reg(wled, QPNP_WLED_HYB_THRES_REG(wled->sink_base), reg);
			if (rc)
...
static int qpnp_wled_flash_en_locked(struct qpnp_wled *wled, int en, int duration)
{
	...
	for (i = 0; i < wled->num_strings; i++) {
		...
		rc = qpnp_wled_write_reg(wled, QPNP_WLED_FS_CURR_REG(wled->sink_base, i), reg);
		if (rc)
			goto exit;

		reg = level & QPNP_WLED_BRIGHT_LSB_MASK;
		rc = qpnp_wled_write_reg(wled, QPNP_WLED_BRIGHT_LSB_REG(wled->sink_base, i), reg);
		if (rc < 0)
			goto exit;

		reg = level >> QPNP_WLED_BRIGHT_MSB_SHIFT;
		reg = reg & QPNP_WLED_BRIGHT_MSB_MASK;
		rc = qpnp_wled_write_reg(wled, QPNP_WLED_BRIGHT_MSB_REG(wled->sink_base, i), reg);
		if (rc < 0)
			goto exit;
	}
...
static struct device_attribute qpnp_wled_attrs[] = {
	...
	/* HTC */
	__ATTR(flash_en, 0664, qpnp_wled_flash_en_show, qpnp_wled_flash_en_store),
};
