...
qpnp_led_masked_write(struct qpnp_led_data *led, u16 addr, u8 mask, u8 val)
{
	...
	rc = regmap_read(led->regmap, addr, &reg);
	if (rc) {
	...
	rc = regmap_write(led->regmap, addr, reg);
	if (rc)
		dev_err(&led->pdev->dev,
...
static void qpnp_dump_regs(struct qpnp_led_data *led, u8 regs[], u8 array_size)
{
	...
	for (i = 0; i < array_size; i++) {
		regmap_read(led->regmap, led->base + regs[i], &val);
		...
		const char *buf, size_t count)
{
	...
	rc = regmap_read(led->regmap, LED_MPP_SINK_CTRL(led->base), &read_data);
		...
		const char *buf, size_t count)
{
	...
	for_each_child_of_node(g_led_virtual->pdev->dev.of_node, node) {
